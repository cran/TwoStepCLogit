...
# problème avec testthat que je ne comprends pas...
...
test_that("the 'formula' argument is validated correctly by Ts.estim", {
      ...
      expect_that(Ts.estim(formula = Y ~ strata(Strata) + cluster(Cluster), data = bison),
          throws_error("at least one covariate must be include in the model"))
      ...
      expect_that(Ts.estim(formula = Y ~ biomass + strata(Strata) + cluster(Cluster + meadow), data = bison),
          throws_error("in 'formula', the cluster must be a single variable"))
...
test_that("the 'formula' argument is validated correctly by ddim", {
      ...
      expect_that(ddim(formula = Y ~ strata(bison[,c("Strata","meadow")]) + cluster(Cluster), data = bison),
          throws_error("in 'formula', the strata must be a single variable"))
})
...
#                     random = ~ bison.nn[,8])
#      # des warnings sont générés, mais la réponse est bonne.
...
# ne fonctionne pas, mais fonctionne si c(rep(FALSE,7), TRUE, FALSE) est donné directement plutôt que de passer par logic8
...
# ne fonctionne pas, mais ce n'est pas grave car il y a plein d'autres façon de spécifier les variables
# et une erreur relativement informative est générée
...
# ces façons d'entrer les arguments ne fonctionnent pas : ok car une erreur relativement informative est générée
